...
class NSClient:
    ...
    def retrieve_resource(self, tenant_id, resource_path, parse_response=True):
        """Retrieves an HTTP REST resource of NetScaler Control Center."""
    ...
    def _tojson(self, obj_dict):
        ...
        return jsonutils.dumps(obj_dict)
    ...
    def _is_valid_response(self, response_status):
        # status is less than 400, the response is fine
    ...
    def _parse_uri(self, uri):
        ...
        if parts.scheme.lower() == "http":
            LOG.warn(_("Connection to NetScaler Control Center (NCC)"
                       "and credentials are sent in clear text 'http' !!"
                       " Please use 'https' scheme when configuring "
                       "NCC URL for stronger security."))
