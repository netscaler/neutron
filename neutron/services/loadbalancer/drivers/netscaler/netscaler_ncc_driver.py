...
class AgentDriver(object):
    def __init__(self, conf):
        ...
        LOG.info(_("NCC configuration found: uri=%s username=%s" % (self.ncc_uri, self.ncc_username)))
    ...
    def create_vip(self, vip, netinfo):
        LOG.info(_("NetScaler Driver received the VIP creation request:vip=%s netinfo=%s" % 
    ...
    def delete_vip(self, vip, netinfo):
        LOG.info(_("NetScaler Driver received the VIP deletion request: vip=%s netinfo=%s" % 
                   (repr(vip), repr(netinfo))))
        self.client.remove_resource("vips/" + vip['id'])

    def create_pool(self, pool, netinfo):
        LOG.info(_("NetScaler Driver received the pool creation request:pool=%s netinfo=%s" % 
    ...
    def update_pool(self, old_pool, pool, old_netinfo, netinfo):
        LOG.info(_("NetScaler Driver received the pool update request:pool=%s netinfo=%s" % 
                    (repr(pool), repr(netinfo))))
        self.client.update_resource("pools/" + old_pool["id"], "pool", pool)

    def delete_pool(self, pool, netinfo):
        LOG.info(_("NetScaler Driver received the pool deletion request:pool=%s netinfo=%s" % 
                    (repr(pool), repr(netinfo))))
        self.client.remove_resource("pools/" + pool['id'])

    def create_member(self, member, netinfo):
        LOG.info(_("NetScaler Driver received the member creation request:member=%s netinfo=%s" % 
    ...
    def update_member(self, old_member, member, old_netinfo, netinfo):
        LOG.info(_("NetScaler Driver received the member update request:old_member=%s member=%s netinfo=%s" % 
    ...
    def delete_member(self, member, netinfo):
        LOG.info(_("NetScaler Driver received a member deletion request:member=%s netinfo=%s" % 
                    (repr(member), repr(netinfo))))
        self.client.remove_resource("members/" + member['id'])

    def create_pool_health_monitor(self, health_monitor, pool_id, netinfo):
        LOG.info(_("NetScaler Driver received the health monitor creation request:health_monitor=%s netinfo=%s" % 
    ...
    def update_health_monitor(self, old_health_monitor, health_monitor, pool_id, netinfo):
        LOG.info(_("NetScaler Driver received the health monitor update request:old_h=%s mealth_monitor health_monitor=%s netinfo=%s" % 
    ...
    def delete_pool_health_monitor(self, health_monitor, pool_id, netinfo):
        LOG.info(_("NetScaler Driver received a health monitor deletion request: health_monitor=%s netinfo=%s" % 
                    (repr(health_monitor), repr(netinfo))))
        self.client.remove_resource("healthmonitors/" + health_monitor['id'])

    def get_stats(self, pool_id):
        LOG.info(_("NetScaler Driver received a stats collection request for pool %s" % pool_id))
        return None


    def get_tasks(self):
        LOG.info(_("NetScaler Driver received a request to check for pending tasks on ncc"))
        return []
