...
TESTPOOL_SNAT_PORT = {
                         'id': TESTPOOL_PORT_ID,
                         'fixed_ips': [
                              {'ip_address': TESTPOOL_SNATIP_ADDRESS}
                         ]
                      }


class FakeDBObject(dict):
    """ Allows its objects attributes to be accessed as a dict. """
...
class TestNetScalerPluginDriver(TestLoadBalancerPluginBase):
    ...
    def setUp(self):
        self.log = mock.patch.object(netscaler_driver, 'LOG').start()
        ...
        self.plugin_instance.drivers[LBAAS_PROVIDER_NAME] = (netscaler_driver.
                                             NetScalerPluginDriver(
    ...
    def test_create_vip(self):
        with self.subnet() as subnet:
            with mock.patch.object(self.driver.plugin._core_plugin,
        ...
        with self.subnet() as subnet:
            with mock.patch.object(self.driver.plugin._core_plugin,
                ...
                with self.pool(provider=LBAAS_PROVIDER_NAME) as pool:
                    ...
                    testvip = self._build_testvip_contents(subnet['subnet'],
                                                       pool['pool'])
                    self.create_resource_mock.side_effect = (exceptions.
                                                       ServiceUnavailable())
                    # execute the method under test
                    # and expect it to throw an exception.
            ...
            with mock.patch.object(self.driver.plugin._core_plugin,
                ...
                with mock.patch.object(self.driver.plugin._core_plugin,
                    ...
                    with mock.patch.object(self.driver.plugin._core_plugin,
                        ...
                        testpool = self._build_testpool_contents(
                                                             subnet['subnet']
                                                                )
                            ...
                            with mock.patch.object(self.driver.plugin,
                                ...
                                with mock.patch.object(self.driver.plugin,
                                   '_delete_db_pool') as mock_delete_db_pool:
                                    ...
                                    self.remove_resource_mock.reset_mock()
                                    # execute the method under test.
                                    self.driver.delete_pool(self.context,
                                                            pool['pool'])
                                    pool_resource_path = "%s/%s" % (
                                             (netscaler_driver.POOLS_RESOURCE,
                                             pool['pool']['id']))
            ...
            with mock.patch.object(self.driver.plugin._core_plugin,
                ...
                with self.pool(provider=LBAAS_PROVIDER_NAME) as pool:
                    with self.member(pool_id=pool['pool']['id']) as member:
                        self.update_resource_mock.reset_mock()
                        updatedmember = (self
                                      ._build_updated_testmember_contents(
                                                             member['member']
                                                                         ))
                        expectedmember = (self
        ...
        with self.subnet() as subnet:
            with mock.patch.object(self.driver.plugin._core_plugin,
                ...
                with self.pool(provider=LBAAS_PROVIDER_NAME) as pool:
                    with self.member(pool_id=pool['pool']['id']) as member:
                        with mock.patch.object(self.driver.plugin,
                                   '_delete_db_member') as (
                                                        mock_delete_db_member):
    ...
    def test_create_pool_health_monitor(self):
        with self.subnet() as subnet:
            with mock.patch.object(self.driver.plugin._core_plugin,
                ...
                with self.pool(provider=LBAAS_PROVIDER_NAME) as pool:
                    self.create_resource_mock.reset_mock()
                    testhealthmonitor = (self
                                          ._build_testhealthmonitor_contents(
                                                                  pool['pool']
                                                                           ))
                    expectedhealthmonitor = (self
                                       ._build_expectedhealthmonitor_contents(
                                                             testhealthmonitor
                                                                            ))
                    # mock the driver's set_member_status() that updates DB
                    # This is easier to mock than trying to mock sqlalchemy
                    # Query/filter_by()/etc. objects.
                    self._mock_set_healthmonitor_status()
                    # execute the method under test.
                    self.driver.create_pool_health_monitor(self.context,
                                                           testhealthmonitor,
                                                           pool['pool']['id'])
                    # unmock the driver's set_member_status() call.
                    self._unmock_set_healthmonitor_status()
                    # First, assert that create_resource was called once
                    # with expected params.
                    resource_path = "%s/%s/%s" % (
                                         netscaler_driver.POOLS_RESOURCE,
                    ...
                    # now ACTIVE.
                    (self.mock_sethealthmonitor_status_obj
    ...
    def test_update_pool_health_monitor(self):
        with self.subnet() as subnet:
            with mock.patch.object(self.driver.plugin._core_plugin,
                ...
                mock_get_subnet.return_value = subnet['subnet']
                with self.pool(provider=LBAAS_PROVIDER_NAME) as pool:
                    with self.health_monitor(pool_id=pool['pool']['id']) as (
                                                               health_monitor):
                        self.update_resource_mock.reset_mock()
                        updatedhealthmonitor = (self
                                    ._build_updated_testhealthmonitor_contents(
                                              health_monitor['health_monitor']
                                                                             ))
                        expectedhealthmonitor = (self
                                ._build_updated_expectedhealthmonitor_contents(
                                                           updatedhealthmonitor
                                                                             ))
                        # mock the driver's set_healthmonitor_status() that
                        # updates DB. This is easier to mock than trying to
                        # mock sqlalchemy Query/filter_by()/etc. objects.
                        self._mock_set_healthmonitor_status()
                        # execute the method under test.
                        self.driver.update_health_monitor(self.context,
                                             health_monitor['health_monitor'],
                                             updatedhealthmonitor,
                                             pool['pool']['id'])
                        # unmock the driver's set_member_status() call.
    ...
    def _build_testvip_contents(self, subnet, pool):
        vip_obj = FakeDBObject()
        vip_obj.id = TESTVIP_ID
        vip_obj.name = 'testvip'
        vip_obj.description = 'a test vip'
        vip_obj.tenant_id = self._tenant_id
        vip_obj.subnet_id = subnet['id']
        vip_obj.port_id = TESTVIP_PORT_ID
        vip_obj.pool_id = pool['id']
        vip_obj.protocol = 'HTTP'
        vip_obj.protocol_port = 80
        vip_obj.connection_limit = 1000
        vip_obj.admin_state_up = True
        vip_obj.status = 'PENDING_CREATE'
        vip_obj.status_description = ''
        return vip_obj

    def _build_expectedvip_contents(self, testvip, subnet):
        expectedvip = FakeDBObject()
        expectedvip.id = testvip.id
        expectedvip.name = testvip.name
        expectedvip.description = testvip.description
        expectedvip.tenant_id = testvip.tenant_id
        expectedvip.subnet_id = testvip.subnet_id
        expectedvip.network_id = subnet['network_id']
        expectedvip.port_id = testvip.port_id
        expectedvip.pool_id = testvip.pool_id
        expectedvip.protocol = testvip.protocol
        expectedvip.protocol_port = testvip.protocol_port
        expectedvip.connection_limit = testvip.connection_limit
        expectedvip.admin_state_up = testvip.admin_state_up
        return expectedvip
    ...
    def _build_testpool_contents(self, subnet):
        pool_obj = FakeDBObject()
        pool_obj.id = TESTPOOL_ID
        pool_obj.name = 'testpool'
        pool_obj.description = 'a test pool'
        pool_obj.tenant_id = self._tenant_id
        pool_obj.subnet_id = subnet['id']
        pool_obj.protocol = 'HTTP'
        pool_obj.protocol_port = 80
        pool_obj.admin_state_up = True
        pool_obj.lb_method = 'ROUND_ROBIN'
        pool_obj.status = 'PENDING_CREATE'
        pool_obj.status_description = ''
        pool_obj["members"] = []
        pool_obj["health_monitors"] = []
        pool_obj["health_monitors_status"] = None
        pool_obj["provider"] = LBAAS_PROVIDER_NAME
        return pool_obj

    def _build_expectedpool_contents(self, testpool, subnet):
        expectedpool = FakeDBObject()
        expectedpool.id = testpool.id
        expectedpool.name = testpool.name
        expectedpool.description = testpool.description
        expectedpool.tenant_id = testpool.tenant_id
        expectedpool.subnet_id = testpool.subnet_id
        expectedpool.network_id = subnet['network_id']
        expectedpool.protocol = testpool.protocol
        expectedpool.protocol_port = testpool.protocol_port
        expectedpool.lb_method = testpool.lb_method
        expectedpool.snat_ip = TESTPOOL_SNATIP_ADDRESS
        expectedpool.port_id = TESTPOOL_PORT_ID
        expectedpool.admin_state_up = testpool.admin_state_up
        return expectedpool
    ...
    def _build_testmember_contents(self, pool):
        member_obj = FakeDBObject()
        member_obj.id = TESTMEMBER_ID
        member_obj.tenant_id = self._tenant_id
        member_obj.pool_id = pool['id']
        member_obj.protocol_port = 8080
        member_obj.weight = 2
        member_obj.admin_state_up = True
        member_obj.status = 'PENDING_CREATE'
        member_obj.status_description = ''
        return member_obj

    def _build_expectedmember_contents(self, testmember):
        expectedmember = FakeDBObject()
        expectedmember.id = testmember.id
        expectedmember.tenant_id = testmember.tenant_id
        expectedmember.pool_id = testmember.pool_id
        expectedmember.protocol_port = testmember.protocol_port
        expectedmember.weight = testmember.weight
        expectedmember.admin_state_up = testmember.admin_state_up
        return expectedmember

    def _build_updated_testmember_contents(self, testmember):
        updated_member = dict(testmember.items())
        updated_member['weight'] = 3
        updated_member['admin_state_up'] = True
        updated_member['status'] = 'PENDING_UPDATE'
        updated_member['status_description'] = ''
    ...
    def _build_testhealthmonitor_contents(self, pool):
        monitor_obj = FakeDBObject()
        monitor_obj.id = TESTMONITOR_ID
        monitor_obj.tenant_id = self._tenant_id
        monitor_obj.pool_id = pool['id']
        monitor_obj.pools = []
        pool_obj = {}
        pool_obj["status"] = "PENDING_CREATE"
        pool_obj["status_description"] = None
        pool_obj["pool_id"] = pool['id']
        monitor_obj.pools.append(pool_obj)
        monitor_obj.type = 'TCP'
        monitor_obj.delay = 10
        monitor_obj.timeout = 5
        monitor_obj.max_retries = 3
        monitor_obj.admin_state_up = True
        return monitor_obj

    def _build_expectedhealthmonitor_contents(self, testhealthmonitor):
        expectedmonitor = FakeDBObject()
        expectedmonitor.id = testhealthmonitor.id
        expectedmonitor.tenant_id = testhealthmonitor.tenant_id
        expectedmonitor.pool_id = testhealthmonitor.pool_id
        expectedmonitor.type = testhealthmonitor.type
        expectedmonitor.delay = testhealthmonitor.delay
        expectedmonitor.timeout = testhealthmonitor.timeout
        expectedmonitor.max_retries = testhealthmonitor.max_retries
        expectedmonitor.admin_state_up = testhealthmonitor.admin_state_up
    ...
    def _build_updated_testhealthmonitor_contents(self, testmonitor):
        ...
        updated_monitor['delay'] = 30
        updated_monitor['timeout'] = 3
        updated_monitor['max_retries'] = 5
        updated_monitor['admin_state_up'] = True
        return updated_monitor

    def _build_updated_expectedhealthmonitor_contents(self, testmonitor):
        expectedmonitor = FakeDBObject()
        expectedmonitor.delay = testmonitor['delay']
        expectedmonitor.timeout = testmonitor['timeout']
        expectedmonitor.max_retries = testmonitor['max_retries']
        expectedmonitor.admin_state_up = testmonitor['admin_state_up']
        return expectedmonitor

    def _mock_update_status(self):
        #patch the driver's set_status() method with a mock object
        self.mock_update_status_patcher = mock.patch.object(self.driver.plugin,
                                                         'update_status')
        self.mock_update_status_obj = self.mock_update_status_patcher.start()

    def _unmock_update_status(self):
        #Stop patching, and return the original method to the plugin.
        self.mock_update_status_patcher.stop()

    def _mock_set_healthmonitor_status(self):
        # patch the driver's set_healthmonitor_status() method
        # with a mock object.
        self.mock_sethealthmonitor_status_patcher = mock.patch.object(
                                               self.driver,
                                               '_set_poolhealthmonitor_status'
                                               )
        self.mock_sethealthmonitor_status_obj = (
                          self.mock_sethealthmonitor_status_patcher.start())

    def _unmock_set_healthmonitor_status(self):
        #Stop patching, and return the original method to the driver.
        self.mock_sethealthmonitor_status_patcher.stop()
